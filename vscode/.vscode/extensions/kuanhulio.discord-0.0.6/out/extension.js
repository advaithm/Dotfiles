...
class Discord {
    tokenChecker() {
        const client = new eris_1.Client(String(token));
        client.on("ready", () => {
            console.log("Ready!");
            if (client.user.bot) {
...
function activate(context) {
    if (token) {
        ...
        vscode.commands.registerCommand('discord.sendMessage', () => {
            vscode.window.showInformationMessage("Loading user accounts is slow, please wait some time.");
            discord.sendMessage();
        });
        vscode.commands.registerCommand('discord.uploadFromPath', () => {
            vscode.window.showInformationMessage("Loading user accounts is slow, please wait some time.");
            discord.uploadFromPath();
        });
        vscode.commands.registerCommand('discord.sendSelectionMessage', () => {
            vscode.window.showInformationMessage("Loading user accounts is slow, please wait some time.");
            discord.sendSelectionMessage();
        });
        vscode.commands.registerCommand('discord.uploadCurrentFile', () => {
            vscode.window.showInformationMessage("Loading user accounts is slow, please wait some time.");
            discord.uploadFromPath();
        });
        vscode.commands.registerCommand('discord.uploadFromCurrentDirectory', () => {
            discord.uploadFromCurrentDirectory();
        });
        vscode.commands.registerCommand('discord.tokenChecker', () => {
            vscode.window.showInformationMessage("Loading user accounts is slow, please wait some time.");
            discord.tokenChecker();
        });
        context.subscriptions.push(discord);
    }
    else {
